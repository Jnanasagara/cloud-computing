...
def run_all(
    ...
    tasks = generate_tasks(
        ...
        cloudy=cloudy,
        use_azure=use_azure,
        azure_filepath=azure_filepath,
    )
    servers = make_cluster(5)
    workload = describe_workload(tasks)
    ...
    for sched in schedulers:
        fresh_tasks = [copy.deepcopy(task) for task in tasks]
        fresh_tasks = [_reset_task(task) for task in fresh_tasks]
        res = sched.run(fresh_tasks, copy.deepcopy(servers))
        results[res.scheduler_name] = res
        print(res.summary())

    return results, tasks, workload
...
def chart_energy_profile(cloudy: bool = False, save: bool = True) -> str:
    ...
    ci = [carbon_intensity(slot, cloudy) for slot in slots]
    ...
    ax2.set_ylabel("Carbon Intensity (gCO2/unit)", color="#e74c3c")
...
def chart_carbon_comparison(results: dict, save: bool = True) -> str:
    ...
    carbons = [results[name].total_carbon_g for name in names]
    colors = [COLORS.get(name, "#7f8c8d") for name in names]
    ...
    ax.set_ylabel("Total Carbon Emissions (gCO2)")
...
def chart_task_distribution(results: dict, save: bool = True) -> str:
    ...
    for name, res in results.items():
        ...
        for start, end in period_slots:
            count = sum(1 for slot, _ in res.schedule_pairs if start <= slot < end)
            counts.append(count)
        data[name] = counts
...
def print_summary_table(results: dict, workload_info: dict):
    print("\n" + "=" * 90)
    print(
        f"{'Scheduler':<20} {'Carbon(gCO2)':>12} {'Tasks':>8} {'DeadMet':>8} "
        f"{'Solar':>8} {'Grid':>8} {'AvgLat(ms)':>12}"
    )
    print("-" * 90)
    for name, res in results.items():
        print(
            f"{name:<20} {res.total_carbon_g:>12.1f} {res.tasks_scheduled:>8} "
...
def run_benchmark(
    ...
    if use_azure:
        print(f"  AzurePath={azure_filepath}")
    print(f"{'=' * 60}\n")

    results, tasks, workload = run_all(
        n_tasks=n_tasks,
        seed=seed,
        cloudy=cloudy,
        use_azure=use_azure,
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Run the carbon-aware scheduler benchmark.")
    parser.add_argument("--tasks", type=int, default=100, help="Number of tasks to schedule.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed.")
    parser.add_argument("--cloudy", action="store_true", help="Use the cloudy-day solar profile.")
    parser.add_argument(
        "--azure",
        action="store_true",
        help="Use Azure VM trace data instead of synthetic task generation.",
    )
    parser.add_argument(
        "--azure-path",
        default="vmtable.csv",
